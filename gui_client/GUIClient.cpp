	...
	*/
	...
	if(false)
	{
		...
		for(size_t q=0; q<test_avatars.size(); ++q)
		{
			...
			assignedLoadedOpenGLTexturesToMats(test_avatar.ptr(), *opengl_engine, *resource_manager);
...
void GUIClient::startLoadingTextureForLocalPath(const std::string& local_abs_tex_path, const Vec4f& centroid_ws, float aabb_ws_longest_len, float max_task_dist, float importance_factor, 
		const TextureParams& tex_params, bool is_terrain_map)
{
				...
				if(mesh_data_loading_progress.done())
				{
					...
					if(voxel_ob->in_proximity)
					{
						...
						if(ob_model_lod_level == cur_loading_voxel_ob_model_lod_level)
						{
							...
							{
								assignedLoadedOpenGLTexturesToMats(voxel_ob.ptr(), *opengl_engine, *resource_manager);
...
std::string GUIClient::getDiagnosticsString(bool do_graphics_diagnostics, bool do_physics_diagnostics, double last_timerEvent_CPU_work_elapsed, double last_updateGL_time)
{
	std::string msg;
					...
					if(this->selected_ob->object_type == WorldObject::ObjectType_Generic || this->selected_ob->object_type == WorldObject::ObjectType_VoxelGroup)
					{
						...
						if(opengl_ob.nonNull())
						{
							if(!opengl_ob->materials.empty())
							{
								...
								assignedLoadedOpenGLTexturesToMats(selected_ob.ptr(), *opengl_engine, *resource_manager);
							}
						}
